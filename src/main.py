...
async def execute_python_chart(payload: PythonCode, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        TEMP_CHART_WORKDIR = "/chart_temp"; temp_volumes = {str(temp_dir_path.resolve()): {'bind': TEMP_CHART_WORKDIR, 'mode': 'rw'}}; command = ["python", f"{TEMP_CHART_WORKDIR}/{script_filename}"]
        exit_code, stdout_str, stderr_str = await run_in_container(command=command, temp_volumes=temp_volumes, working_dir=TEMP_CHART_WORKDIR, network_mode="none")
        logger.info(f"Chart Execution: Container stdout:\n{stdout_str}");
        if stderr_str: logger.warning(f"Chart Execution: Container stderr:\n{stderr_str}")
        if exit_code != 0: logger.error(f"Chart Execution: Script failed with exit code {exit_code}."); error_detail = f"Python script execution failed (Exit Code: {exit_code})."; log_preview = '\n'.join(stderr_str.splitlines()[-10:]); error_detail += f"\nStderr (Last 10 lines):\n{log_preview}"; raise HTTPException(status_code=400, detail=error_detail)
        logger.info(f"Chart Execution: Checking for output file at host path: {output_path_host}")
        if not output_path_host.is_file(): logger.error(f"Chart Execution: Output file '{output_path_host}' not found despite exit code 0."); error_detail = f"Script executed successfully but failed to produce the expected output file ('{OUTPUT_FILENAME}')."; log_preview_stdout = '\n'.join(stdout_str.splitlines()[-10:]); log_preview_stderr = '\n'.join(stderr_str.splitlines()[-10:]); error_detail += f"\nStdout (Last 10 lines):\n{log_preview_stdout}"; error_detail += f"\nStderr (Last 10 lines):\n{log_preview_stderr}"; raise HTTPException(status_code=500, detail=error_detail)
...
async def execute_python_script(payload: PythonScript):
    ...
    if not payload.session_id: raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="session_id cannot be empty.")
    script_filename = "script.py"; script_path_in_container = f"{WORKSPACE_DIR_INSIDE_CONTAINER}/{script_filename}"
    try: # Write script
        escaped_code = shlex.quote(payload.code); write_command = f"printf '%s' {escaped_code} > {script_filename}"; write_command_list = ["bash", "-c", f"set -e; {write_command}"]
        logger.info(f"Python Script (Session: {payload.session_id}): Attempting to write script file via shell...")
        write_exit_code, write_stdout, write_stderr = await run_in_container(command=write_command_list, session_id=payload.session_id, environment=None, working_dir=WORKSPACE_DIR_INSIDE_CONTAINER, network_mode="none")
    ...
    try: # Execute script
        ...
        exec_exit_code, exec_stdout, exec_stderr = await run_in_container(command=exec_command_list, session_id=payload.session_id, environment=payload.environment, working_dir=WORKSPACE_DIR_INSIDE_CONTAINER, network_mode="bridge")
